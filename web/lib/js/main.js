        ...
        if(g_last_command != data.id){
            ...
            if(!controlRemote) {
                ...
                if (data.action == "goToFavourite") {
                    if(g_favourites[g_current_country] != "" && g_current_country != null && stations[g_current_country] && stations[g_current_country].length > 0) {
                        var chosen_country = g_current_country;
                        var index = stations[chosen_country].map(function (e) {
                            return e.name;
                        }).indexOf(g_favourites[chosen_country]);
                        if(index < 0) index = 0;
                        if(index < stations[chosen_country].length && g_current_url == stations[chosen_country][index]["url"]){
                            ...
                            for(var key in g_countries){
                                if(g_countries.hasOwnProperty(key)){
                                    ...
                                    if(!stations[key] || stations[key].length === 0) continue;
                                    if(new_country == null){
                            ...
                            if(chosen_country && stations[chosen_country] && stations[chosen_country].length > 0) {
                        ...
                        if(index < 0) index = 0;
                        if(chosen_country && stations[chosen_country] && index < stations[chosen_country].length && g_countries[chosen_country]) {
        ...
        if (!paused || !g_initial_received) {
            ...
            if (!available_countries.hasOwnProperty(g_current_country) ||
                ...
                $.getJSON(g_api + "/favourite/" + country_best_reception, function (favourite_lowest_distance) {
                    // Guard: Check if stations data exists for this country
                    if (!stations[country_best_reception] || stations[country_best_reception].length === 0) {
                        refreshStations();
                        return;
                    }
                    ...
                    if (index >= stations[country_best_reception].length) {
                        index = 0;
                    }
                    if (!controlRemote) {
...
function setRadioStation(url, country, volume) {
    ...
    if(controlRemote){
        ...
        // Set audio source IMMEDIATELY (before animations)
        if (cleanUrl.endsWith("m3u8") || url.endsWith("m3u8")) {
            ...
            g_hls.on(Hls.Events.MEDIA_ATTACHED, function () {
                ...
                document.getElementById("player").play().catch(function(e) {
    ...
    // Play audio (source is now already set)
    document.getElementById('player').play().catch(function(e) {
        console.log("Player play error:", e);
    });
    document.getElementById('whitenoise').play();

    // Guard: Only access stations if data is valid
    if (stations[g_current_country] && stations[g_current_country].length > 0) {
        var index = stations[g_current_country].map(function (e) {
            return e.url;
        }).indexOf(g_current_url);

        if (index >= 0 && g_countries[country]) {
...
function setCurrentAsFavourite() {
    if(g_current_country != null && stations[g_current_country] && stations[g_current_country].length > 0) {
        var index = stations[g_current_country].map(function (e) {
            return e.url;
        }).indexOf(g_current_url);
        if(index != -1) {
...
function volumeChange(amount) {
    ...
    // Apply volume immediately to player
    document.getElementById("player").volume = g_volume;
